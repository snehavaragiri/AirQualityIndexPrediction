...
def avg_data_2013():
    temp_i=0
    average=[]
    # we have 12 months in  Data/AQI/aqi2013 file.. so iterate through them. 
    # We will choose 24 records in 1st iteration.Therefore, chunksize = 24
    for rows in pd.read_csv('Data/AQI/aqi2013.csv',chunksize=24):
        add_var=0   # To add hourly data
        avg=0.0     #avg of 24 hrs data
        data=[]
        df=pd.DataFrame(data=rows)   # convert 24 hours into dataframe for ease.
        # iterate trhough 24 rows and and take the row PM2.5 and append to the list.
        for index,row in df.iterrows():
            data.append(row['PM2.5'])
        for i in data:
            if type(i) is float or type(i) is int:   # if the values in the list are float/int just add them.
                add_var=add_var+i
            # if the value is not equal to NoData/pwrFail/invld/--- then convert it to float add them to list.(it can be for ex "12")
            elif type(i) is str:      
